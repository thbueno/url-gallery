...
describe("categorize", () => {
  const cases: [string | undefined, string | undefined, string, string[]][] = [
  ...
  it.each(cases)("categorize(%s, %s, %s) → %j", (type, siteName, domain, expected) => {
    expect(categorize(type, siteName, domain)).toEqual(expected)
  })
})
